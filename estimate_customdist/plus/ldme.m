...
szp = 100; % pooling size
m = .5; % mean of norm dist
s = .1; % std of norm dist
l = .9; % lambda of exp dist
a = .3; % proportion of norm
b = 1-a; % propotion of exp
...
dist_sample = dist_sample / length(data); % normalize
sum(dist_sample)

figure
hold on
bar(p, dist_sample)
line = plot(p, mycustom(p', m, s, l, a, b), 'r');
...
hold on
line = plot(p, mycustom(p', para_guess(1), para_guess(2), para_guess(3), para_guess(4), para_guess(5)), 'k');
set(line, 'LineWidth', 2)
hold off
